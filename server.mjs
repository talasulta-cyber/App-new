...
<section class="panel"><h2>إنشاء ترخيص جديد</h2><div class="grid"><div><label>اسم المستخدم</label><input id="userName" placeholder="اسم صاحب المحل"></div><div><label>رقم المستخدم</label><input id="userNumber" placeholder="رقم الهاتف أو الرقم الداخلي"></div><div><label>المدة</label><select id="plan"><option value="monthly">شهري</option><option value="yearly">سنوي</option><option value="lifetime">دائم</option><option value="custom">مخصص</option></select></div><div><label>عدد الأجهزة</label><select id="maxDevices"><option value="1">جهاز واحد</option><option value="2">جهازان</option><option value="3" selected>3 أجهزة</option><option value="4">4 أجهزة</option><option value="5">5 أجهزة</option></select></div><div><label>تاريخ الانتهاء للمخصص</label><input id="expiresAt" type="date"></div></div><button onclick="createLicense()">توليد الكود وحفظ الترخيص</button></section>
<section class="panel" id="created" hidden><h2>الكود الجديد</h2><p>احفظ الكود وأرسله للمستخدم:</p><span class="code" id="createdCode"></span> <button onclick="copyCode()">نسخ الكود</button><span id="copyState" class="muted"></span></section>
...
async function createLicense(){try{const plan=el('plan').value;let expiresAt=el('expiresAt').value||null;if(plan==='lifetime')expiresAt=null;const data=await api('/v1/admin/licenses',{method:'POST',body:JSON.stringify({userName:el('userName').value,userNumber:el('userNumber').value,plan,maxDevices:Number(el('maxDevices').value),expiresAt})});el('createdCode').textContent=data.activationCode;el('created').hidden=false;el('copyState').textContent='';el('userName').value='';el('userNumber').value='';await loadLicenses()}catch(e){notice(e.message)}}
async function copyCode(){const code=el('createdCode').textContent;if(!code)return;try{await navigator.clipboard.writeText(code);el('copyState').textContent=' تم النسخ'}catch{const area=document.createElement('textarea');area.value=code;document.body.appendChild(area);area.select();document.execCommand('copy');area.remove();el('copyState').textContent=' تم النسخ'}}
...
app.post("/v1/admin/licenses", async (req, res) => {
  ...
  if (!Number.isInteger(maxDevices) || maxDevices < 1 || maxDevices > 5 || (expiresAt && Number.isNaN(expiresAt.getTime()))) {
